...
Avant de vous lancer dans ce cours, vous devez installer et vous familiariser avec la SciViews Box `r learnitdown$svbox`. Si ce n'est pas encore fait, retournez à la [page d'accueil pour bien débuter]( `r learnitdown$baseurl` ).

RStudio et (R) Markdown ne doivent plus avoir de secrets pour vous. 

##### À vous de jouer ! {-}

`r h5p(19, height = 150, toc = NULL)`
...
`r h5p(19, height = 149, toc = NULL)`


L'ensemble de ces outils a été abordé dans les cours précédents [SDD I](`r paste(learnitdown$baseurl, "sdd-umons", sep = "/")`), [SDD II](`r paste(learnitdown$baseurl, "sdd-umons2", sep = "/")`) et SDD III (les six premiers modules du présent cours) que vous devez avoir réalisé avant d'entamer la matière du cours SDD IV\ : pratique.
...
`r h5p(19, height = 149, toc = "Nombre de jours dans une année")`
...
if (exists("assignation"))
  assignation("D07Ia_21M_data", part = "I",
...
- qui se répète plusieurs fois\ : 
      ...
      + **{2,6}** répétitions comprises entre 2 et 6 fois
...
```{r, echo=FALSE, results='asis'}
if (exists("assignation"))
  assignation("D07Ia_21M_data", part = "II",
    url = "https://github.com/BioDataScience-Course/D07Ia_21M_data",
    course.urls = c(
      'S-BIOG-043' = "https://classroom.github.com/a/mmcdbtb6"),
